...
ADD https://googledrive.com/host/0Bz-lYS0FYZbIMXFWazlnRFpqbFE/openerp-7.0-20140328-001256.tar.gz /opt/odoo/odoo.tar.gz
...
RUN /bin/bash -c "mkdir -p /opt/odoo/{bin,etc,sources/odoo/addons,sources/odoo/openerp/filestore,additional_addons,data}" && \
...
WORKDIR /app
VOLUME ["/opt/odoo/var", "/opt/odoo/etc", "/opt/odoo/additional_addons", "/opt/odoo/data", "/opt/odoo/sources/odoo/openerp/filestore"]
