...
RUN /bin/bash -c "mkdir -p /opt/odoo/{bin,etc,sources/odoo/addons,sources/odoo/openerp/addons/filestore,additional_addons,data}" && \
...
WORKDIR /app
VOLUME ["/opt/odoo/var", "/opt/odoo/etc", "/opt/odoo/additional_addons", "/opt/odoo/data", "/opt/odoo/sources/odoo/openerp/addons/filestore"]
